...
import { cancelHouseholdPrintJob, getPrintBatches } from '../lib/api';
...
const ATTENTION = new Set(['awaiting_refeed', 'uncertain', 'failed']);
...
function BatchRow({ job, scope, fresh, busy, onCancel }) {
  ...
  return <li className="batch-list-row">
    ...
    <div className="batch-list-status">
      <span className={`batch-list-badge${ATTENTION.has(job.state) ? ' batch-list-badge--attention' : ''}${job.state === 'completed' ? ' batch-list-badge--done' : ''}`}>{STATES[job.state] || 'Status unavailable'}</span>
      {detail && <p>{detail}</p>}
    ...
    <div className="batch-list-actions">
      ...
      {scope === 'all' && job.canCancel && <button type="button" className="batch-list-cancel" disabled={!fresh || busy} onClick={event => { event.currentTarget.focus(); onCancel(job); }}>Cancel waiting batch</button>}
    </div>
  </li>;
}
...
function BatchHistory({ state, order, onBusyChange }) {
  ...
  function chooseCancel(job) { modalRef.current = job; setCancelBatch(job); }
  function dismissCancel() { modalRef.current = null; setCancelBatch(null); }
  async function cancel() {
    const job = modalRef.current;
    if (!job || mutationRef.current) return;
    dismissCancel();
    if (loading || error || !fetchedAt || Date.now() - fetchedAt > 60_000 || data?.scope !== 'all') {
    ...
    try {
      await cancelHouseholdPrintJob(job.id);
      if (mountedRef.current) setNotice(`Canceled ${job.deckName || 'Card batch'} · ${job.totalCopies} copies · batch ${job.id}.`);
    } catch (failure) {
  ...
  return <>
    ...
    {cancelBatch && <ConfirmModal title="Cancel this waiting batch?" message={`${cancelBatch.deckName || 'Card batch'} · ${cancelBatch.totalCopies} copies. Batch ${cancelBatch.id}. Cancel only this batch before it is sent to Epson. Pages already submitted to Epson cannot be canceled here.`} confirmLabel="Cancel this batch" cancelLabel="Keep batch" danger onConfirm={cancel} onCancel={dismissCancel} />}
  </>;
}
