...
function NavItem({ item, route, compact, onNavigate }) {
  ...
  return <a href={item.href} className={`shell-nav-item${active ? ' is-active' : ''}`} aria-current={active ? 'page' : undefined} onClick={onNavigate}><Icon name={item.icon} /><span>{compact ? item.short || item.label : item.label}</span></a>;
}
