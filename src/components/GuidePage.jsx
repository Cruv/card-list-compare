...
function Printing() {
  return <div className="guide-section">
    ...
    <p><strong>Waiting in CLC</strong> means the batch is saved here and has not been sent to Epson. <strong>In the Epson queue</strong> identifies a pass already submitted to the Mac spooler. All front images finish first. Double-faced backs stay in Backs for later while other front jobs continue. Spooler completed is a receipt, not approval of the physical output.</p>
    <p>Owners and admins can choose <strong>Cancel batch</strong> to stop remaining fronts and backs, or <strong>Cancel only backs</strong> to keep the fronts. Cancellation never undoes paper already printed. If an affected pass is active, wait for the Mac to stop that exact submission, clear its paper and confirm that only blank paper is in the rear feeder. An uncertain submission still requires Mac reconciliation. Canceled passes stay recorded and are not retried.</p>
    ...
    <h3>Double-faced sheets: print backs whenever you are ready</h3>
    <p>With companion 2.55.0 or newer, new jobs print every front first and save unfinished backs under <strong>Printer → Backs for later</strong>. Other front jobs continue. Keep the labeled sheets together by job; the batch and packet identity connect each sheet to its matching back. Saved backs survive restarts and remain available on another day until completed or canceled.</p>
    <p>Choose <strong>Prepare this packet’s backs</strong> for the sheet you want. Keep blank paper loaded while the Mac finishes its current work and reserves the printer. Only when Printer says the selected packet is ready to reload should you remove unused blank paper, match the printed job/packet label, flip and reload <strong>only that packet’s sheet</strong> using your verified orientation. Older multi-sheet packets must stay together in their tested order.</p>
    <p>Choose <strong>Confirm this paper is reloaded</strong>, check the acknowledgement and choose <strong>Confirm and print this packet’s backs</strong>. Afterward, remove the printed or flipped paper, restore blank paper, check the paper-clearance acknowledgement and choose <strong>Confirm blank paper is ready</strong>. Other front jobs resume only after that step. Cancel remaining backs if they are no longer needed; completed fronts stay recorded. Drying, lamination and cutting remain household tasks. Older in-progress jobs keep their original sequence until finished.</p>
    ...
    <p><strong>Printer settings</strong> groups Print recipe, Discord printer alerts and Companion updates. Admins can connect, save, explicitly test or disconnect Discord. With companion 2.54.0 or newer, Proxy Balboa sends a message naming the whole job when all its print passes have completed in the Mac spooler. Completion and test messages never mention a user directly. The optional Discord user ID is used only for alerts that need help, such as flipping paper or correcting a printer error. Jobs already completed before the upgrade are not announced again. Spooler completion does not confirm the physical quality or usability of the cards.</p>
    <p>Fronts-finished messages announce saved backs without a personal mention. A flip alert mentions you only after a packet you selected has reserved the printer; a paper-clearance alert requests blank paper before other jobs resume. Alerts identify the deck or list, batch and packet. Companion 2.53 or newer also checks the local printer queue about once a minute and alerts on reported paper, jam, offline, stopped or failed-pass conditions. New faults alert once per error episode; a healthy check permits alerts if the fault returns. Unknown status does not reset the episode. Two consecutive failed printer checks raise a separate status-unavailable alert without implying recovery. Alerts depend on what CUPS and the Epson driver report, and never pause, resume or retry printing.</p>
    <p>Proxy Balboa’s Discord messages lead with the real event, add a short Rocky-inspired line, then give plain details: job name, printer, full batch ID, packet, sheet count, error and next action as applicable. <strong>Send test message</strong> sends a real channel message without requesting printing. Update controls require a compatible managed companion and an idle station.</p>
