...
- **Printing metadata preservation** &mdash; set codes, collector numbers (including promos like `136p`, `DDO-20`), and foil status survive the full round-trip (import &rarr; diff &rarr; export)
- **Cross-source metadata carry-forward** &mdash; comparing an Archidekt snapshot against a DeckCheck/plain text import? The Archidekt export inherits printing metadata from the richer source automatically
